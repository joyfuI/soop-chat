...
`0007 status=0/1`은 방송 중에도 반복되며 정확한 의미는 미확인입니다. 최신 공식 플레이어도 수신 분기에서 값을 읽거나 UI로 전달하지 않고 무시합니다. 방송인 플래그 계정의 채팅 채널 입퇴장과 같은 시각에 오는 사례가 많고 채팅창 새로고침 구간에서도 관찰됐지만, 구체적인 UI 동작을 판정하는 신호로 정의하지 않습니다. 원본 숫자만 제공하고 영상 송출·대기·화면·종료 상태로 해석하지 않습니다. 방송 대기와 채팅창 얼음도 실제 종료와 독립적입니다.
