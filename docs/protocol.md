...
`0008 setDumb`는 채팅금지 대상, 지속 시간, 누적 횟수와 명령 주체를 전달합니다. 같은 사용자가 방송인에 의해 30초·1회 채팅금지를 받은 직후 매니저에게 다시 제재돼 60초·2회로 늘어난 사례와, 다른 사용자가 방송인에게 30초 채팅금지를 받은 사례를 패킷 및 별도 확인 경로와 대조했습니다. 공식 플레이어도 지속 시간을 초 단위로 사용하며 `commanderType=1`을 방송인, `2`를 매니저로 표시합니다. 플레이어가 무시하는 다섯 번째 필드는 실제 표본에서 명령 주체의 SOOP ID와 일치해 `commanderId`로 제공합니다.
