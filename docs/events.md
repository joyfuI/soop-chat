...
방송 밖에서 VOD에 받은 별풍선을 다음 라이브 입장 시 합계로 알리는 이벤트입니다. 화면의 VOD 별풍선 2,894개·1개 표시와 패킷의 `balloonCount=2894/1`을 각각 대조했습니다.
...
실방송에서 `count=2/4`가 화면의 “[닉네임] 방송국 애드벌룬 2개/4개”와 각각 일치했습니다. 공식 플레이어도 애드벌룬 공통 UI에서 채널 출처에 “방송국”을 붙입니다. 패킷의 `title`에는 더 긴 문장이 오므로 화면 문구를 새 필드로 합성하지 않고 원문 그대로 제공합니다.
