...
test("uses UTF-8 byte lengths and parses split packets", () => {
  ...
  assert.equal(declaredLength, new TextEncoder().encode(`${separator}한글😀${separator}user`).length);
...
test("parses multiple packets and recovers after garbage", () => {
  ...
  assert.deepEqual(result.packets.map((packet) => packet.opcode), ["0000", "9999"]);
  assert.equal(decodePacket(result.packets[1]!).type, "unknown");
});

test("accepts every WebSocket message data representation", async () => {
  const value = "hello😀";
...
test("catalog exposes 101 known opcodes plus the unknown variant", () => {
...
test("decodes chat, subscription, broadcaster status, and current player fields", () => {
  const joined = decodePacket(rawPacket("0004", `${separator}1${separator}user1${separator}nick1${separator}16${separator}user2${separator}nick2${separator}32`));
  ...
  const left = decodePacket(rawPacket("0004", `${separator}-1${separator}user${separator}nick${separator}2${separator}etc${separator}16`));
  ...
  const normallyLeft = decodePacket(rawPacket("0004", `${separator}-1${separator}user${separator}nick${separator}1${separator}-1${separator}16`));
  ...
  const chat = decodePacket(rawPacket("0005", `${separator}hello${separator}user${separator}${separator}1${separator}2${separator}nick${separator}flag${separator}9`));
  ...
  const dumb = decodePacket(rawPacket("0008", `${separator}target${separator}ignored${separator}30${separator}2${separator}streamer${separator}1${separator}${separator}targetNick`));
  ...
  const userFlag = decodePacket(rawPacket("0012", `${separator}528${separator}user${separator}nick${separator}0${separator}0${separator}16`));
  assert.equal(userFlag.type, "setUserFlag");
  if (userFlag.type === "setUserFlag") assert.equal(userFlag.data.previousUserFlag, "16");

  const nickname = decodePacket(rawPacket("0014", `${separator}user${separator}newNickname${separator}${separator}65536|163840${separator}oldNickname`));
  ...
  const ice = decodePacket(rawPacket("0021", `${separator}1${separator}1${separator}528${separator}1${separator}1`));
  ...
  const managerChat = decodePacket(rawPacket("0026", `${separator}line1\rline2${separator}sender${separator}1${separator}ignored${separator}manager${separator}512${separator}3`));
  ...
  for (const [itemType, quickViewProduct, durationDays] of [
    [1, "quickView", 30], [2, "quickView", 90], [3, "quickView", 365],
    [100, "quickViewPlus", 7], [101, "quickViewPlus", 30], [102, "quickViewPlus", 90], [103, "quickViewPlus", 365],
    [7, "unknown", null],
  ] as const) {
    const quickView = decodePacket(rawPacket("0045", `${separator}ignored${separator}sender${separator}sNick${separator}receiver${separator}rNick${separator}${itemType}`));
  ...
  const poll = decodePacket(rawPacket("0050", `${separator}1${separator}bj${separator}123${separator}1`));
  ...
  const closedPoll = decodePacket(rawPacket("0050", `${separator}4${separator}bj${separator}123${separator}1`));
  const hiddenPoll = decodePacket(rawPacket("0050", `${separator}2${separator}bj${separator}123${separator}0`));
  ...
  const balloon = decodePacket(rawPacket("0018", `${separator}bj${separator}user${separator}nick${separator}10${separator}0${separator}0${separator}123${separator}10${separator}0${separator}0${separator}google_tts${separator}synthetic-id${separator}ko_KR${separator}456`));
  ...
  const manager = decodePacket(rawPacket("0013", `${separator}user${separator}256|0${separator}1${separator}manager`));
  ...
  const vodBalloon = decodePacket(rawPacket("0086", `${separator}bj${separator}sender${separator}nickname${separator}2894${separator}bj_2894${separator}0${separator}7373${separator}ko_KR${separator}456`));
  ...
  const adcon = decodePacket(rawPacket("0087", `${separator}123${separator}bj${separator}user${separator}nick${separator}message${separator}secondary${separator}title${separator}image${separator}default${separator}10${separator}20${separator}1${separator}0${separator}0${separator}${separator}${separator}ko_KR${separator}456`));
  ...
  const follow = decodePacket(rawPacket("0093", `${separator}bj${separator}user${separator}nick${separator}4${separator}123${separator}100${separator}8${separator}1${separator}synthetic-id${separator}ko_KR${separator}456`));
  ...
  const plusFollow = decodePacket(rawPacket("0093", `${separator}bj${separator}user${separator}nick${separator}30${separator}123${separator}200${separator}30${separator}2${separator}synthetic-id${separator}ko_KR${separator}456`));
  assert.equal(plusFollow.type, "followItemEffect");
  if (plusFollow.type === "followItemEffect") assert.equal(plusFollow.data.subscriptionTier, "plus");
  ...
  if (kickMessageState.type === "kickMsgState") assert.equal(kickMessageState.data.hideKickMessage, true);

  const translated = decodePacket(rawPacket("0095", `${separator}7${separator}1${separator}translated${separator}2${separator}3`));
  ...
  const ticket = decodePacket(rawPacket("0102", `${separator}ignored${separator}sender${separator}sNick${separator}receiver${separator}rNick${separator}ticket-data`));
  assert.equal(ticket.type, "giftTicket");
  if (ticket.type === "giftTicket") assert.equal(ticket.data.ticketData, "ticket-data");

  const gift = decodePacket(rawPacket("0108", `${separator}unused${separator}sender${separator}sNick${separator}receiver${separator}rNick${separator}streamer${separator}streamerNick${separator}11${separator}code${separator}0${separator}0${separator}${separator}0${separator}0`));
  ...
  const video = decodePacket(rawPacket("0105", `${separator}1${separator}bj${separator}sender${separator}nickname${separator}50${separator}2${separator}${separator}0${separator}0${separator}${separator}${separator}${separator}${separator}1`));
  assert.equal(video.type, "videoBalloon");
  if (video.type === "videoBalloon") assert.equal(video.data.becameFanClub, true);

  const stationAdcon = decodePacket(rawPacket("0107", `${separator}bj${separator}user${separator}nick${separator}10${separator}image${separator}title${separator}123${separator}ko_KR${separator}456`));
  assert.equal(stationAdcon.type, "stationAdcon");
  if (stationAdcon.type === "stationAdcon") assert.equal(stationAdcon.data.title, "title");

  const battleMission = decodePacket(rawPacket("0121", `${separator}{"type":"NOTICE","uuid":"synthetic"}`));
  ...
  const challengeGift = decodePacket(rawPacket("0121", `${separator}{"type":"CHALLENGE_GIFT","key":123,"uuid":"gift-event","title":"미션","gift_count":500,"chno":456,"is_relay":false,"image":"gift-image","user_id":"sender","user_nick":"senderNick","bj_id":"bj","bj_nick":"bjNick"}`));
  assert.equal(challengeGift.type, "mission");
  if (challengeGift.type === "mission" && challengeGift.data.missionKind === "challenge" && challengeGift.data.action === "gift") {
  ...
  const challengeNotice = decodePacket(rawPacket("0121", `${separator}{"type":"CHALLENGE_NOTICE","key":123,"uuid":"notice-event","title":"미션","mission_status":"SUCCESS"}`));
  assert.equal(challengeNotice.type, "mission");
  if (challengeNotice.type === "mission" && challengeNotice.data.missionKind === "challenge" && challengeNotice.data.action === "notice") {
    assert.equal(challengeNotice.data.status, "success");
    assert.equal(challengeNotice.data.missionKey, 123);
  }

  const challengeSettle = decodePacket(rawPacket("0121", `${separator}{"type":"CHALLENGE_SETTLE","key":123,"uuid":"settle-event","title":"미션","settle_count":1000,"is_relay":false,"image":"settle-image","bj_id":"bj","bj_nick":"bjNick"}`));
  assert.equal(challengeSettle.type, "mission");
  if (challengeSettle.type === "mission" && challengeSettle.data.missionKind === "challenge" && challengeSettle.data.action === "settle") {
  ...
  const missionSettle = decodePacket(rawPacket("0125", `${separator}{"chno":456,"uuid":"settle-event","fanOrder":18002,"list":[["user","nickname",1000,1,0]]}`));
  ...
  const ogq = decodePacket(rawPacket("0109", `${separator}123${separator}이미지와 함께 표시${separator}group${separator}20${separator}1${separator}user${separator}nickname${separator}flag${separator}1122867${separator}-1${separator}0${separator}png${separator}4${separator}123456${separator}654321${separator}8${separator}-1${separator}0${separator}-1`));
  ...
  const ogqGift = decodePacket(rawPacket("0118", `${separator}ignored${separator}sender${separator}sNick${separator}receiver${separator}rNick${separator}title${separator}image`));
  ...
  if (inBroadcastAd.type === "adInBroadJson") assert.equal(inBroadcastAd.data.payload.ad, "synthetic");

  const gem = decodePacket(rawPacket("0120", `${separator}ignored${separator}receiver${separator}rNick${separator}gem-item`));
  assert.equal(gem.type, "gemItemSend");
  if (gem.type === "gemItemSend") assert.equal(gem.data.itemName, "gem-item");

  const extendedUsers = decodePacket(rawPacket("0127", `${separator}user1${separator}p=3&fw=4&afw=8${separator}user2${separator}fw=-1&afw=-1`));
  ...
  const subtitle = decodePacket(rawPacket("0136", `${separator}1${separator}streamer${separator}ko${separator}안녕하세요${separator}123`));
  assert.equal(subtitle.type, "globalSubtitle");
  if (subtitle.type === "globalSubtitle") assert.equal(subtitle.data.subtitle, "안녕하세요");

  const timeout = decodePacket(rawPacket("0141", `${separator}user${separator}nickname${separator}2${separator}123${separator}message${separator}60${separator}flag`));
  assert.equal(timeout.type, "nightbotTimeout");
  if (timeout.type === "nightbotTimeout") assert.equal(timeout.data.reasonCode, 2);
});

test("decodes every field-reading official player branch", () => {
  const cases: readonly [string, string, string, Record<string, unknown>][] = [
    ...
    ["0002", `${separator}123${separator}bj${separator}1${separator}10${separator}2]family${separator}ignored${separator}16|0`, "joinChannel", { chatNo: "123", broadcasterId: "bj", maxManagerCount: 10, familyNickname: "family", familyNicknamePosition: 2 }],
    ["0003", `${separator}ignored${separator}ignored${separator}2${separator}3${separator}admin${separator}bannedBj${separator}bannedNick`, "quitChannel", { kickType: 2, actor: "manager", adminKickCount: 3 }],
    ["0009", `${separator}line1\rline2${separator}sender${separator}receiver${separator}3${separator}2${separator}senderNick${separator}receiverNick${separator}1|0`, "directChat", { message: "line1line2", senderId: "sender", receiverId: "receiver", isAdmin: true }],
    ["0020", `${separator}bj${separator}bjNick${separator}sender${separator}senderNick${separator}ignored${separator}1${separator}ignored${separator}10${separator}2${separator}ignored${separator}ignored${separator}ko_KR`, "sendFanLetter", { broadcasterNickname: "bjNick", count: 10, relay: false }],
    ["0023", `${separator}5${separator}10`, "slowMode", { automaticSeconds: 5, manualSeconds: 10 }],
    ["0033", `${separator}ignored${separator}bj${separator}bjNick${separator}sender${separator}senderNick${separator}10${separator}2${separator}ignored${separator}file${separator}1${separator}3${separator}tts${separator}ignored${separator}ko_KR${separator}456`, "sendBalloonSub", { broadcasterId: "bj", senderId: "sender", count: 10, relay: true }],
    ["0034", `${separator}ignored${separator}bj${separator}bjNick${separator}sender${separator}senderNick${separator}ignored${separator}1${separator}ignored${separator}10${separator}2${separator}ignored${separator}ko_KR`, "sendFanLetterSub", { senderNickname: "senderNick", count: 10, relay: true }],
    ["0037", `${separator}ignored${separator}bj${separator}sender${separator}senderNick${separator}10`, "chocolate", { broadcasterId: "bj", count: 10, relay: false }],
    ["0038", `${separator}ignored${separator}bj${separator}sender${separator}senderNick${separator}10`, "chocolateSub", { count: 10, relay: true }],
    ["0047", `${separator}ignored${separator}bj${separator}1${separator}121`, "itemUsing", { remainingSeconds: 121, remainingMinutes: 2 }],
    ["0070", `${separator}ignored${separator}1${separator}bj${separator}ignored${separator}buyer${separator}buyerNick${separator}goods${separator}2`, "buyGoods", { goodsType: 1, buyerId: "buyer", goodsName: "goods", count: 2, relay: false }],
    ["0071", `${separator}ignored${separator}1${separator}bj${separator}ignored${separator}buyer${separator}buyerNick${separator}goods${separator}2`, "buyGoodsSub", { count: 2, relay: true }],
    ["0074", `${separator}1${separator}bj${separator}vr${separator}rtmp${separator}hls${separator}2`, "notifyVr", { action: 1, broadcasterId: "bj", vrType: 2 }],
    ["0075", `${separator}0`, "notifyMobBroadPause", { state: 0, action: "pause" }],
    ["0076", `${separator}1${separator}user${separator}nickname`, "kickAndCancel", { cancelled: true, userId: "user" }],
    ["0092", `${separator}123${separator}bj${separator}sender${separator}nickname${separator}message${separator}secondary${separator}title${separator}image${separator}default${separator}10`, "itemSellEffect", { chatNo: 123, title: "title", count: 10 }],
    ["0103", `${separator}bj${separator}sender${separator}nickname${separator}10${separator}image${separator}title${separator}123${separator}ko_KR${separator}456`, "vodAdcon", { broadcasterId: "bj", chatNo: "123", count: 10 }],
    ["0111", `${separator}ignored${separator}bj${separator}drop${separator}message${separator}image`, "itemDrops", { broadcasterId: "bj", name: "drop", imageUrl: "image" }],
  ...
  const kickList = decodePacket(rawPacket("0077", `${separator}user${separator}nickname${separator}time${separator}manager${separator}managerNick${separator}256|1024`));
  assert.equal(kickList.type, "kickUserList");
  if (kickList.type === "kickUserList") assert.partialDeepStrictEqual(kickList.data.users[0], { userId: "user", commanderPrimaryFlag: 256, commanderSecondaryFlag: 1024 });

  const adminUsers = decodePacket(rawPacket("0078", `${separator}1${separator}employee${separator}employeeNick${separator}0|1024${separator}admin${separator}adminNick${separator}1|0${separator}manager${separator}managerNick${separator}256|0`));
  assert.equal(adminUsers.type, "adminChatUser");
  if (adminUsers.type === "adminChatUser") assert.deepEqual(adminUsers.data.users.map((user) => user.role), ["employee", "admin", "manager"]);

  const chat = decodePacket(rawPacket("0005", `${separator}line1\rline2${separator}sender${separator}1122867${separator}0${separator}3${separator}nickname${separator}16|0${separator}4${separator}light${separator}dark${separator}8${separator}2${separator}ignored${separator}7`));
  assert.equal(chat.type, "chatMessage");
  if (chat.type === "chatMessage") assert.partialDeepStrictEqual(chat.data, { message: "line1line2", color: "#332211", accumulatedSubscriptionMonth: "8", representativePersonalconMonth: "2", cheerTeamNumber: 7 });

  const follow = decodePacket(rawPacket("0091", `${separator}123${separator}receiver${separator}sender${separator}nickname${separator}111${separator}ignored${separator}ignored${separator}1${separator}ignored${separator}ko_KR${separator}456`));
  assert.equal(follow.type, "followItem");
  if (follow.type === "followItem") assert.partialDeepStrictEqual(follow.data, { tier: 1, subscriptionTier: "basic", subscriptionMonth: 1, senderLanguage: "ko_KR", urlModify: "456" });

  const battle = decodePacket(rawPacket("0121", `${separator}{"type":"GIFT","title":"battle","gift_count":100,"is_relay":true,"image":"image","user_id":"sender","user_nick":"nickname","fan_order":10,"top_fan":2}`));
  assert.equal(battle.type, "mission");
  if (battle.type === "mission" && battle.data.missionKind === "battle" && battle.data.action === "gift") assert.partialDeepStrictEqual(battle.data, { giftCount: 100, senderId: "sender", fanOrder: 10, topFanLevel: 2 });

  const timeout = decodePacket(rawPacket("0141", `${separator}user${separator}nickname${separator}4${separator}123${separator}message${separator}60${separator}flag`));
  assert.equal(timeout.type, "nightbotTimeout");
  if (timeout.type === "nightbotTimeout") assert.equal(timeout.data.reason, "links");
});

test("connects subscription item types to the official player product table", () => {
  const regularGift = decodePacket(rawPacket("0108", `${separator}unused${separator}sender${separator}sNick${separator}receiver${separator}rNick${separator}streamer${separator}streamerNick${separator}103${separator}code${separator}0${separator}0${separator}${separator}0${separator}0`));
  assert.equal(regularGift.type, "sendSubscription");
  if (regularGift.type === "sendSubscription") {
    assert.partialDeepStrictEqual(regularGift.data.subscriptionProduct, { subscriptionTier: "basic", level: 1, month: 3, isGift: true, isTrial: false, isLegacy: false });
  }

  const trialGift = decodePacket(rawPacket("0108", `${separator}unused${separator}sender${separator}sNick${separator}receiver${separator}rNick${separator}streamer${separator}streamerNick${separator}30${separator}code${separator}0${separator}0${separator}${separator}0${separator}0`));
  assert.equal(trialGift.type, "sendSubscription");
  if (trialGift.type === "sendSubscription") {
    assert.partialDeepStrictEqual(trialGift.data.subscriptionProduct, { subscriptionTier: "plus", level: 1, month: 1, isCeremony: true, isTrial: true });
  }

  const legacyGift = decodePacket(rawPacket("0108", `${separator}unused${separator}sender${separator}sNick${separator}receiver${separator}rNick${separator}streamer${separator}streamerNick${separator}1${separator}code${separator}0${separator}0${separator}${separator}0${separator}0`));
  assert.equal(legacyGift.type, "sendSubscription");
  if (legacyGift.type === "sendSubscription") {
    assert.partialDeepStrictEqual(legacyGift.data.subscriptionProduct, { month: 3, isLegacy: true, isCeremony: false, isGift: true });
  }

  const legacySubscription = decodePacket(rawPacket("0091", `${separator}123${separator}receiver${separator}sender${separator}nickname${separator}1${separator}ignored${separator}ignored${separator}1${separator}ignored${separator}ko_KR${separator}456`));
  assert.equal(legacySubscription.type, "followItem");
  if (legacySubscription.type === "followItem") {
    assert.partialDeepStrictEqual(legacySubscription.data.subscriptionProduct, { month: 3, isLegacy: true, isCeremony: false, isGift: true });
  }

  const plusSubscription = decodePacket(rawPacket("0091", `${separator}123${separator}receiver${separator}sender${separator}nickname${separator}2413${separator}ignored${separator}ignored${separator}2${separator}ignored${separator}ko_KR${separator}456`));
  assert.equal(plusSubscription.type, "followItem");
  if (plusSubscription.type === "followItem") {
    assert.partialDeepStrictEqual(plusSubscription.data.subscriptionProduct, { subscriptionTier: "plus", level: 4, month: 1, isGift: false });
  }

  const vodSubscription = decodePacket(rawPacket("0091", `${separator}123${separator}receiver${separator}sender${separator}nickname${separator}92013${separator}ignored${separator}ignored${separator}2${separator}ignored${separator}ko_KR${separator}456`));
  assert.equal(vodSubscription.type, "followItem");
  if (vodSubscription.type === "followItem") {
    assert.partialDeepStrictEqual(vodSubscription.data.subscriptionProduct, { itemType: 2013, vodItemType: 92013, subscriptionTier: "plus", level: 1 });
  }

  const unknown = decodePacket(rawPacket("0108", `${separator}unused${separator}sender${separator}sNick${separator}receiver${separator}rNick${separator}streamer${separator}streamerNick${separator}999${separator}code${separator}0${separator}0${separator}${separator}0${separator}0`));
...
test("rejects malformed specialized payloads without losing raw parsing", () => {
  ...
  const malformedSettlement = rawPacket("0125", `${separator}{"fanOrder":1,"list":[["too","short"]]}`);
